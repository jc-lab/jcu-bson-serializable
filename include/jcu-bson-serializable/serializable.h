...
namespace jcu {
    namespace bson {
        ...
        class SerializableSharedptrCreateFactory;
        class SerializableSharedptrCreateFactory
        {
        public:
            virtual std::shared_ptr<Serializable> create() = 0;
            virtual std::shared_ptr<Serializable> create(const std::string& name, int64_t serialVersionUID) { return NULL; }
        };
        ...
        namespace internal {
            ...
            protected:
                ...
                SerializableSharedptrCreateFactory *createSharedptrFactory;
                bool _isnull;
            ...
            public:
                ...
                STypeCommon &setCreateFactory(SerializableSharedptrCreateFactory *factory) {
                    this->createSharedptrFactory = factory;
                    return *this;
                }
                ...
                SerializableSharedptrCreateFactory *getSerializableSharedptrCreateFactory() {
                    return this->createSharedptrFactory;
                }
                ...
                virtual uint32_t deserialize(uint8_t type,
            ...
            public:
                ...
                virtual bool bsonParseHandle(uint8_t type,
                                             const std::string &name,
                                             const std::vector<unsigned char> &payload,
                                             uint32_t *offset,
                                             uint32_t docEndPos) = 0;
            };
        }
        ...
        public:
            uint32_t serialize(std::vector<unsigned char> &payload) const override {
                ...
                return internal::ObjectHelper<internal::IsSerializableClass<T>::Result, T>::serialize(payload,
                                                                                                      this->key,
                                                                                                      this->object);
            }

            uint32_t deserialize(uint8_t type,
                                 const std::vector<unsigned char> &payload,
                                 uint32_t *offset,
                                 uint32_t documentSize) override {
                ...
                this->_isnull = false;
                return internal::ObjectHelper<internal::IsSerializableClass<T>::Result, T>::deserialize(this,
                                                                                                        object,
                                                                                                        type,
                                                                                                        payload,
                                                                                                        offset,
        ...
        public:
            ...
            static bool readMetadata(const std::vector<unsigned char> &payload,
                                     size_t offset,
                                     std::string *pName = NULL,
                                     int64_t *pSerialVersionUID = NULL,
                                     uint32_t *pDocSize = NULL);
        ...
        namespace internal {
            ...
            public:
                BsonParser(const std::vector<unsigned char> &_payload,
                           uint32_t rootDocSize,
                           uint32_t *rootDocOffset,
                           uint32_t deserializationConfigs) : payload(_payload) {
            ...
            template<typename T>
            class IsSerializableSharedptrClass {
            public:
                enum { Result = 0 };
            };
            ...
            template<typename D>
            class IsSerializableSharedptrClass< std::shared_ptr<D> >
            {
                class No { };
                class Yes { No no[3]; };
            ...
            public:
                ...
                IsSerializableSharedptrClass() { void(*p)(D*) = Constraints; }
            };
            ...
            public:
                enum {
                    Result =
                    (sizeof(Test(static_cast<D *>(0))) == sizeof(Yes)) ? 1
                                                                       : IsSerializableSmartpointerClass<D>::Result
                };
...
#define _JSBSONRPC_MAKESERIALIZE_BASICSMALLTYPE(TYPE, SERTYPE, INT32TYPE, INT64TYPE, BSONTYPE) \
        ...
        struct ObjectHelper<0, TYPE> { \
            ...
            struct ObjectHelper<0, float> {
                static uint32_t serialize(std::vector<unsigned char> &payload,
                ...
                static uint32_t deserialize(internal::STypeCommon *rootSType,
                                            float &object,
                                            uint8_t type,
                                            const std::vector<unsigned char> &payload,
                                            uint32_t *offset,
            ...
            struct ObjectHelper<0, std::list<T> > : public BsonParseHandler {
                ...
                ObjectHelper(internal::STypeCommon *_rootSType, std::list<T> &object)
                    : rootSType(_rootSType), refObject(object) {}
                ...
                static uint32_t serialize(std::vector<unsigned char> &payload,
                    ...
                    payload.push_back(0);
                    payload.push_back(0);
                    for (typename std::list<T>::const_iterator iter = object.begin(); iter != object.end();
                         iter++) {
                        // doucment
                        _my_itoa(i, seqKey, sizeof(seqKey), 10);
                        subDocumentSize +=
                            ObjectHelper<internal::IsSerializableClass<T>::Result, T>::serialize(payload,
                                                                                                 seqKey,
                                                                                                 *iter);
                        i++;
                    }
                ...
                bool bsonParseHandle(uint8_t type,
                                     ...
                                     uint32_t docEndPos) override {
                    T temp;
                    ObjectHelper<internal::IsSerializableClass<T>::Result, T>::deserialize(rootSType,
                                                                                           temp,
                                                                                           type,
                                                                                           payload,
                                                                                           offset,
            ...
            struct ObjectHelper<0, std::map<std::string, T> > : public BsonParseHandler {
                ...
                ObjectHelper(internal::STypeCommon *_rootSType, std::map<std::string, T> &object)
                    : rootSType(_rootSType), refObject(object) {}

                static uint32_t serialize(std::vector<unsigned char> &payload,
                                          const std::string &key,
                                          const std::map<std::string, T> &object) {
                    size_t offset;
                    uint32_t payloadLen = 1;
                    uint32_t subDocumentSize = 5;
                    ...
                    payload.push_back(0);
                    payload.push_back(0);
                    for (typename std::map<std::string, T>::const_iterator iter = object.begin();
                         iter != object.end();
                         iter++) {
                        // doucment
                        subDocumentSize +=
            ...
            template<typename T>
            struct ObjectHelper<1, std::shared_ptr<T> > {
                static uint32_t serialize(std::vector<unsigned char> &payload, const std::string& key, const std::shared_ptr<T> &object) {
                    size_t payloadLen = 1;
                    if (!object) {
                        return serializeNullObject(payload, key);
                    }
                ...
                static uint32_t deserialize(internal::STypeCommon *rootSType, std::shared_ptr<T> &object, uint8_t type, const std::vector<unsigned char> &payload, uint32_t *offset, uint32_t documentSize) {
                    if (rootSType->getSerializableSharedptrCreateFactory())
                    {
                        std::string sname;
                        int64_t sver;
                        Serializable::readMetadata(payload, *offset, &sname, &sver);
                        std::shared_ptr<Serializable> newObj = rootSType->getSerializableSharedptrCreateFactory()->create(sname, sver);
                        if (!newObj)
                            newObj = rootSType->getSerializableSharedptrCreateFactory()->create();
                        if (newObj) {
                            object = std::move(newObj);
                        }
                    }
                ...
                static void objectClear(std::shared_ptr<T> &object) {
                    object.reset();
                }
            };
        }
