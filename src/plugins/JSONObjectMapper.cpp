...
namespace jcu {
    namespace bson {
            ...
            static uint32_t jsonArrayToBson(std::vector<unsigned char> &payload,
                                            const std::string &key,
                                            const rapidjson::Value &jsonObject);
            static uint32_t jsonObjectToBson(std::vector<unsigned char> &payload,
                                             const std::string &key,
                                             const rapidjson::Value &jsonObject);

            static uint32_t addJsonValueToBson(std::vector<unsigned char> &payload,
                                               const std::string &key,
                ...
                if (jsonValue.IsInt64()) {
                    ...
                    payload.insert(payload.end(),
                                   jsonValue.GetString(),
                                   jsonValue.GetString() + jsonValue.GetStringLength());
                    payload.push_back(0);
                    payloadSize += 4 + len;
            ...
            static uint32_t jsonObjectToBson(std::vector<unsigned char> &payload,
                ...
                payload.push_back(0);
                payload.push_back(0);
                for (rapidjson::Value::ConstMemberIterator iter = jsonObject.MemberBegin();
                     iter != jsonObject.MemberEnd();
                     iter++) {
                    subDocSize += addJsonValueToBson(payload,
            ...
            void JSONObjectMapper::serializeTo(const Serializable *serialiable,
                                               rapidjson::Document &jsonDoc) throw(TypeNotSupportException, ConvertException) {
                ...
                internal::BsonParser
                    parser(bsonPayload, bsonPayload.size(), &offset, DeserializationConfig::getDefaultConfigure());
                parser.parse(&convertContext);
            }

            void JSONObjectMapper::deserializeJsonObject(Serializable *serialiable,
                                                         const rapidjson::Value &jsonObject) throw(TypeNotSupportException, ConvertException) {
            ...
            void JSONObjectMapper::ConvertContext::serializableNameHandle(const std::string &attrName,
            ...
            void JSONObjectMapper::ConvertContext::serializableSerialVersionUIDHandle(const std::string &attrName,
                                                                                      int64_t value) {
                rapidjson::Value jsonKey;
                rapidjson::Value jsonValue;
                jsonKey.SetString(attrName.c_str(), attrName.length(), doc.GetAllocator());
            ...
            bool JSONObjectMapper::ConvertContext::bsonParseHandle(uint8_t type,
                                                                   const std::string &name,
                                                                   const std::vector<unsigned char> &payload,
                                                                   uint32_t *offset,
                                                                   uint32_t docEndPos) throw(TypeNotSupportException, ConvertException) {
                rapidjson::Value jsonKey;
                rapidjson::Value jsonValue;
                ...
                switch (type) {
                    case internal::BSONTYPE_DOUBLE:
                        jsonValue.SetDouble(internal::readValue<double>(payload,
                                                                        offset,
                                                                        docEndPos));
                        break;
                    ...
                    case internal::BSONTYPE_DOCUMENT: {
                        ...
                        internal::BsonParser
                            parser(payload, payload.size(), offset, DeserializationConfig::getDefaultConfigure());
                        parser.parse(&convertContext);
                    ...
                    case internal::BSONTYPE_BOOL:
                        jsonValue.SetBool(internal::readValue<unsigned char>(payload,
                                                                             offset,
                                                                             docEndPos) ? true : false);
                        break;
                    case internal::BSONTYPE_UTCDATETIME:
                        jsonValue.SetUint64(internal::readValue<uint64_t>(payload,
                                                                          offset,
                                                                          docEndPos));
                        break;
                    case internal::BSONTYPE_NULL: jsonValue.SetNull();
                        break;
                    case internal::BSONTYPE_INT32:
                        jsonValue.SetInt(internal::readValue<int32_t>(payload,
                                                                      offset,
                                                                      docEndPos));
                        break;
                    ...
                    case internal::BSONTYPE_INT64:
                        jsonValue.SetInt64(internal::readValue<int64_t>(payload,
                                                                        offset,
                                                                        docEndPos));
                        break;
                    default: throw TypeNotSupportException();
                }
