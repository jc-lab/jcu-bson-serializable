...
#include "jcu-bson-serializable/serializable.h"
...
namespace jcu {
    namespace bson {
        ...
        uint32_t DeserializationConfig::getDefaultConfigure() {
            BuildContext *buildContext = getBuildContext();
            uint32_t mask = 0;
            for (std::list<DeserializationConfig>::const_iterator iter = buildContext->list.begin();
                 iter != buildContext->list.end(); iter++) {
                mask |= iter->getMask();
            }
            return mask;
        }
        ...
        internal::STypeCommon &Serializable::serializableMapMember(const char *name,
                                                                   internal::STypeCommon &object) {
        ...
        void Serializable::serializableClearObjects() {
            for (std::list<internal::STypeCommon *>::const_iterator iter = m_members.begin();
                 iter != m_members.end();
                 iter++) {
                (*iter)->clear();
            }
        ...
        size_t Serializable::serialize(std::vector<unsigned char> &payload) const throw(UnavailableTypeException) {
            ...
            totalSize +=
                internal::ObjectHelper<0, std::string>::serialize(payload, "@jsbsonrpcsname", this->m_name);
            totalSize +=
                internal::ObjectHelper<0, int64_t>::serialize(payload, "@jsbsonrpcsver", this->m_serialVersionUID);

            for (std::list<internal::STypeCommon *>::const_iterator iterMem = m_members.begin();
                 iterMem != m_members.end(); iterMem++) {
        ...
        size_t Serializable::deserialize(const std::vector<unsigned char> &payload,
                                         size_t offset) throw(ParseException) {
            uint32_t tempOffset = offset;
            internal::BsonParser parser(payload, payload.size(), &tempOffset, m_deserializationConfigs);
            return parser.parse(this);
        }

        bool Serializable::bsonParseHandle(uint8_t type,
        ...
        namespace internal {
            void dummyRead(const std::vector<unsigned char> &payload,
                           uint32_t *offset,
                           uint32_t docEndPos,
                           uint8_t type) {
                switch (type) {
                    case BsonTypes::BSONTYPE_DOUBLE: *offset += 8;
                        break;
                    ...
                    case BsonTypes::BSONTYPE_DOCUMENT:
                        *offset += readValue<uint32_t>(payload, offset, docEndPos) - 4;
                        break;
                    case BsonTypes::BSONTYPE_ARRAY: *offset += readValue<uint32_t>(payload, offset, docEndPos) - 4;
                        break;
                    case BsonTypes::BSONTYPE_BINARY: *offset += 1 + readValue<uint32_t>(payload, offset, docEndPos);
                        break;
                    case BsonTypes::BSONTYPE_UTCDATETIME: *offset += 8;
                        break;
                    case BsonTypes::BSONTYPE_BOOL: *offset += 1;
                        break;
                    case BsonTypes::BSONTYPE_NULL: break;
                    case BsonTypes::BSONTYPE_INT32: *offset += 4;
                        break;
                    case BsonTypes::BSONTYPE_TIMESTAMP: *offset += 8;
                        break;
                    case BsonTypes::BSONTYPE_INT64: *offset += 8;
                        break;
                    default: throw Serializable::ParseException();
                        break;
                }
            }
        }
        ...
        uint32_t internal::BsonParser::parse(BsonParseHandler *handler) {
            ...
            while ((docEndPos - *offset) > 0) {
                ...
                if (ename == "@jsbsonrpcsname") {
                    std::string sname;
                    internal::ObjectHelper<0, std::string>::deserialize(NULL,
                                                                        sname,
                                                                        type,
                                                                        payload,
                                                                        offset,
        ...
        bool Serializable::readMetadata(const std::vector<unsigned char> &payload,
                                        size_t offset,
                                        std::string *pName,
                                        int64_t *pSerialVersionUID,
                                        uint32_t *pDocSize) {
            uint32_t docSize;
